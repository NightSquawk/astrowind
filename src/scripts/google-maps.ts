 ...
 */
function createMap(mapId: string, location: google.maps.LatLngLiteral, config: MapConfig, locationName?: string) {
  const mapEl = document.getElementById(mapId);
  if (!mapEl) return;
    ...
    try {
      ...
      service.findPlaceFromQuery(request, (results, status) => {
        if (status === google.maps.places.PlacesServiceStatus.OK && results && results[0]) {
          ...
          if (location) {
            createMap(mapId, { lat: location.lat(), lng: location.lng() }, config, place.formatted_address);
            resolve(true);
          } else {
            resolve(false);
          }
    ...
    try {
      ...
      geocoder.geocode({ address: address }, (results, status) => {
        if (status === 'OK' && results && results[0]) {
          ...
          createMap(mapId, { lat: location.lat(), lng: location.lng() }, config, results[0].formatted_address);
          resolve(true);
        } else {
