...
let cachedPromoRedirects: Record<string, Redirect> | null = null;
...
export async function getPromoRedirects(): Promise<Record<string, Redirect>> {
  if (cachedPromoRedirects) return cachedPromoRedirects;
  ...
  try {
    ...
    for (const campaign of campaigns) {
      const campaignSlug = getContentSlug(campaign);
      ...
      if (campaign.data.shortUrl && startDate <= todayStartPacific && endDate >= todayStartPacific) {
        const shortUrl = campaign.data.shortUrl.startsWith('/') ? campaign.data.shortUrl : `/${campaign.data.shortUrl}`;

        redirects[shortUrl] = {
          destination: `/campaigns/${campaignSlug}/`,
          permanent: false,
          category: 'promo',
          utmParams: campaign.data.utmParams || {
            utm_source: 'website',
            utm_medium: 'promo',
            utm_campaign: campaign.data.id || campaignSlug,
          },
        };
      }
    }
    ...
    for (const coupon of coupons) {
      const couponSlug = getContentSlug(coupon);
      ...
      if (coupon.data.shortUrl && expirationDate >= todayStartPacific) {
        const shortUrl = coupon.data.shortUrl.startsWith('/') ? coupon.data.shortUrl : `/${coupon.data.shortUrl}`;

        // Determine destination based on destinationType
        let destination = `/coupon/${couponSlug}/`;
      ...
      if (coupon.data.campaignLinks && expirationDate >= todayStartPacific) {
        ...
        const campaignLinks = coupon.data.campaignLinks as Record<
          string,
          { shortUrl: string; utmParams?: Redirect['utmParams'] }
        >;

        for (const [campaignType, campaignLink] of Object.entries(campaignLinks)) {
  ...
  cachedPromoRedirects = redirects;
  return redirects;
}
